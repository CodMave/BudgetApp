...
class _Profile extends State<Profile>{//private class of the Profile class

  Uint8List? _image;//initialize the image variable
  static final  FirebaseAuth _auth=FirebaseAuth.instance;
  final String _imagekey='user_image';//initialize the _imagekey to hold default value to the image

  PickImage(ImageSource source)async{//Allow the user to pick an image to change the profile
    final ImagePicker _imagepicker=ImagePicker();
    XFile? _file=await _imagepicker.pickImage(source: source);
    if(_file!=null){
      return await _file.readAsBytes();
  ...
  void loadStoredImage()async{//load the profile image when the user load the app
    SharedPreferences prefs=await SharedPreferences.getInstance();
    String? imageData=prefs.getString(_imagekey);
    if(imageData!=null){
      setState(() {
  ...
  void saveImageToStorage(Uint8List image)async{
    SharedPreferences prefs=await SharedPreferences.getInstance();
    String imageData=base64Encode(image);//again encode the String to the image
    prefs.setString(_imagekey,imageData);

  }

  void SelectImageFromGalery() async {//allows user to select an image from the gallery
    Uint8List img = await PickImage(ImageSource.gallery);
    if(img!=null) {
      saveImageToStorage(img);//save the image
  ...
  static Future<String> getUserName() async {
    ...
    if (user != null) {
      ...
      QuerySnapshot qs = await FirebaseFirestore.instance
          ...
          .get();
=======
  static Future <String> getUserName()async {//get the username of the current user and display it as text
    User? user = _auth.currentUser;
    email = user!.email!;
    if (user != null) {//the query check wither the authentication email match with the email which is taken at the user details
...
=======
  ...
  static Future <String> getCurrency()async {//get the currency that user selected and show it as text
    User? user = _auth.currentUser;
    email = user!.email!;
    if (user != null) {
      QuerySnapshot qs = await FirebaseFirestore.instance.collection(//the query check wither the authentication email match with the email which is taken at the user details
          'userDetails').where('email', isEqualTo:email).limit(1).get();
>>>>>>> main
...
=======
  ...
  Widget buttonsheet(context){//show button sheet when the user click on the camera allows to set a new image
    return Container(
      height:100,
...
=======
              TextButton.icon(onPressed: (){//camera option to set the image
                TakePhoto();
              },
                icon: Icon(Icons.camera),
                label: Text('camera'),
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.start,
            children: [
              Container(//displays the top header
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.start,
                  children: [
                    Container(
...
=======
                            child: IconButton(
                              icon: Icon(
                                Icons.arrow_back_ios_new,//when uer click on it he can move to the home page
                                size: 40,
                                color: Colors.white,
                              ),
                              onPressed: () {
...
=======
                              ...
                              children: [
                                Container(
                                  ...
                                  child: Stack(
                                    alignment: Alignment.center,
                                    children: [
                                      _image!=null ?//if the user hasn't set an image yet then display this image in the following circle avatar
                                      CircleAvatar(
                                          radius: 80.0,
                                          backgroundImage:
                                          MemoryImage(_image!)
                                          ...
                                          :CircleAvatar(//if the user set an image then display the corresponding image in the following circle avatar
                                          radius: 80.0,
                                          backgroundImage:
                                      ...
                                      Positioned(
                                        ...
                                        child: InkWell(
                                          ...
                                          child: Icon(
                                            Icons.camera_alt,//camera icon allows user to set an image
                                            color: Colors.teal,
                                            size: 28.0,
              ...
              Column(
                ...
                children: [
                  ...
                  Container(//this container display the current user's name as text
                    width:300,


                    margin: EdgeInsets.only(left:60,right: 60,top:10),
                    padding: EdgeInsets.only(top:20.0,left:10,right:10,bottom:20.0),
                    decoration: BoxDecoration(
                      border: Border.all(
                        color: Colors.black,
                        width: 0.0,
                  ...
                  Container(//this container displays the user's currency as the text
                    width:300,
                    ...
                    children: [
                      Container(
                        ...
                        child:ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(//a button set to allow the user to move to the settings page
                      ...
                      Container(//this button allows to user to log out from the account
